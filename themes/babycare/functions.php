<?php
/**
 * Babycare functions and definitions
 *
 * @package Babycare
 */
...
function petcare_lite_setup() {
	...
	add_image_size('babycare-homepage-thumb',true);
	register_nav_menus( array(
		'primary' => __( 'Primary Menu', 'babycare' ),		
	) );
...
function petcare_lite_widgets_init() {
	register_sidebar( array(
		'name'          => __( 'Blog Sidebar', 'babycare' ),
		'description'   => __( 'Appears on blog page sidebar', 'babycare' ),
		'id'            => 'sidebar-1',
...
function petcare_lite_font_url(){
		...
		*/
		$mont = _x('on', 'Montserrat font:on or off','babycare');
		
		/* Translators: If there are any character that are
		* not supported by Sintony, translate this to off, do not
		* translate into your own language.
		*/
		$sintony = _x('on', 'Sintony font:on or off','babycare');
...
function petcare_lite_scripts() {
	wp_enqueue_style( 'babycare-font', petcare_lite_font_url(), array() );
	wp_enqueue_style( 'babycare-basic-style', get_stylesheet_uri() );
	wp_enqueue_style( 'babycare-responsive-style', get_template_directory_uri().'/css/theme-responsive.css' );
	wp_enqueue_style( 'nivo-style', get_template_directory_uri().'/css/nivo-slider.css');
	wp_enqueue_script( 'jquery-nivo-slider-js', get_template_directory_uri() . '/js/jquery.nivo.slider.js', array('jquery') );
	wp_enqueue_script( 'babycare-customscripts', get_template_directory_uri() . '/js/custom.js', array('jquery') );
